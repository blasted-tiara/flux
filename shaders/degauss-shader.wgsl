...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let time = f32(global.tick) / 60.;
    let strength = 5.0;
    let decay = strength * exp(-time * 0.5); // fades out over ~1 second
    ...
    let freq = mix(30.0, 5.0, min(time / 1.0, 1.0));

    // Add asymmetry: horizontal and vertical warp differ
    let wave_x = sin(radius * freq + time * 8.0) * 0.015;
    let wave_y = cos(radius * (freq * 0.8) - time * 6.0) * 0.015;

    // Add noise-based irregularity
    let n = noise(uv * 3.0 + vec2<f32>(time * 0.5, time * 0.7)) - 0.5;
