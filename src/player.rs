...
impl Player {
    ...
    pub fn handle_input(&mut self) {
        ...
        match self.movement_status {
            MovementStatus::IsLanded => {
                if jump_just_pressed || self.jump_buffer_timer > 0 {
                    self.actor.velocity.y = -self.jump_force;
        ...
        if gp.left.pressed() {
            self.actor.add_velocity(Vector2::new(-self.acceleration, 0.0));
            ...
            if self.actor.velocity.x > 0. {
                self.actor.add_velocity(Vector2::new(-self.deceleration, 0.0));
            } else if self.actor.velocity.x < 0. {
                self.actor.add_velocity(Vector2::new(self.deceleration, 0.0));
            }
        }

        self.actor.clamp_velocity_x(Vector2::new(-self.move_speed_max, self.move_speed_max));
        if self.movement_status != MovementStatus::IsLanded {
            let current_gravity = if self.movement_status == MovementStatus::IsFalling && !jump_pressed
                ...
                } else if self.actor.velocity.y < 25. {
                    Vector2::new(0., self.gravity / 3.)
                } else { 
                    Vector2::new(0.,  self.gravity )
                };
            self.actor.add_velocity(current_gravity);
            self.actor.clamp_velocity_y(Vector2::new(-self.jump_force, self.max_gravity));
        }
    ...
    pub fn check_collision_tilemap(&mut self, tiles: &[Tile]) {
        ...
        match check_collision(&self.actor.position + &Vector2::new(0.0, self.actor.velocity.y), Direction::Down, tiles) {
            Some(tile) => {
        ...
        // Check collision up
        if self.actor.velocity.y < 0.0 {
            while self.actor.velocity.y < 0.0 {
                match check_collision(&self.actor.position + &Vector2::new(0.0, self.actor.velocity.y), Direction::Up, tiles) {
                    Some(_) => { self.actor.add_velocity(Vector2::new(0.0, 1.0)); }
                    None => { break; }
                }
            }
        }
        ...
        if self.actor.velocity.x > 0.0 {
            while self.actor.velocity.x > 0.0 {
                match check_collision(&self.actor.position + &Vector2::new(self.actor.velocity.x, 0.0), Direction::Right, tiles) {
                    Some(_) => { self.actor.add_velocity(Vector2::new(-1.0, 0.0)); }
                    None => { break; }
                }
            }
        }
        ...
        if self.actor.velocity.x < 0.0 {
            while self.actor.velocity.x < 0.0 {
                match check_collision( &self.actor.position + &Vector2::new(self.actor.velocity.x, 0.0), Direction::Left, tiles) {
                    Some(_) => { self.actor.add_velocity(Vector2::new(1.0, 0.0)); }
                    None => { break; }
                }
            }
    ...
    fn snap_to_tile(self: &mut Self, tile: &Tile, direction: Direction) {
        match direction {
            Direction::Down => {
                self.actor.position.y = tile.top();
            }
            _ => {}
        }
    }
    ...
    pub fn draw(&self) {
        if self.movement_status == MovementStatus::IsLanded && self.actor.velocity.x != 0. {
            sprite!(
                "kiwi_walking",
                x = self.actor.position.x as i32,
                y = self.actor.position.y as i32,
